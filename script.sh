...
KERNEL_DIR=$PWD
HOMEPATH=/home/seba3567
KERNEL_DEFCONFIG=evert_defconfig
...
echo " "
echo -e "$yellow        #  DeluxeKernel/GoldenKernel  #"
echo -e "$cyan ______________________"
...
export KBUILD_BUILD_USER=Seba_3567
...
cp $KERNEL_DIR/out/arch/arm64/boot/Image.gz $ZIP_DIR/seba/
############################################################################################################################################################

echo -e "$yellow // - Copying dtb - //"
cp $KERNEL_DIR/out/arch/arm64/boot/dtb $ZIP_DIR/seba/
...
echo -e "$yellow Deluxe • KERNEL Build completed in $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) seconds."
############################################################################################################################################################
